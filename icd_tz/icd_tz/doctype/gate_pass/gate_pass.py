...
def set_expiry_datetime(self):
	...
	try:
		...
		expiry_hours = int(
			settings.expiry_hours or 72
		)  # Default to 72 hours if not set

		# Calculate expiry datetime from submission datetime
		submission_datetime = get_datetime(
			f"{self.submitted_date} {self.submitted_time}"
		)
		...
		frappe.logger().error(
			f"Error setting expiry datetime for Gate Pass {self.name}: {str(e)}"
		)
		# Set default expiry (72 hours from now) if there's an error
		default_expiry = add_to_date(
			get_datetime(f"{self.submitted_date} {self.submitted_time}"), hours=72
		)
...
def auto_expire_gate_passes():
  ...
  for gp in expired_gate_passes:
    try:
      ...
      doc.add_comment(
        "Comment",
        f"Auto-cancelled due to expiry. Gate Pass expired on {expiry_datetime.strftime('%Y-%m-%d at %H:%M:%S')}. Container was not moved out within the agreed time stored in ICD settings.",
      )
      ...
      frappe.logger().info(
        f"Auto-cancelled expired Gate Pass: {gp.name} for container {gp.container_no}. Expired: {expiry_datetime}"
      )

    except Exception as e:
      frappe.logger().error(
        f"Failed to auto-cancel Gate Pass {gp.name}: {str(e)}"
      )

  if cancelled_count > 0:
    frappe.logger().info(
      f"Auto-expiry process completed. Cancelled {cancelled_count} expired Gate Passes."
    )
    ...
    try:
      ...
      pass  # Ignore if realtime doesn't work
