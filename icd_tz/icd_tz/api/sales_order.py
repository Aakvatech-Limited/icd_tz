...
def get_storage_services(m_bl_no=None, h_bl_no=None):
    ...
    for container in containers:
        ...
        # REMOVAL CHARGE (optional - use "Reefer-Removal" if you wish to specialize this fee)
        if (not container_doc.r_sales_invoice and 
            container_doc.has_removal_charges == "Yes"):
            service_type = f"{service_prefix}-Removal"
            service_item = None
            # Logic to get service_item from settings (similar as above)
            # If no Reefer-Removal set, fallback to normal removal
